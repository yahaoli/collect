      ...
      <div>
        <el-table  ref="multipleTable" @select-all="checked=!checked" @selection-change="handleSelectionChange" class="admin-table" border   max-height="400"   :data="tableData">
          ...
          <el-table-column align="center" prop="owner" label="商品所属"></el-table-column>
            <el-table-column align="center" prop="goodsStatus" label="商品状态"></el-table-column>
          <el-table-column align="center" prop="warehouseType" label="仓储方式">
            <template slot-scope="scope">
              {{$common.dataCommon().storage[scope.row.warehouseType]}}
            </template>
          </el-table-column>
          <el-table-column align="center" prop="handle" label="操作">
            <template slot-scope="scope">
              <el-button v-if="$common.havePower(49)" @click="handleClick(scope.row,1)" type="text" size="mini">查看</el-button>
              <template  v-if="scope.row.goodsStatus==='未上架'">
                <el-button v-if="$common.havePower(121)" @click="handleClick(scope.row,2)"  type="text" size="mini">修改</el-button>
                <el-button v-if="scope.row.goodsProperty!=='虚拟质押'&&scope.row.goodsProperty!=='质押商品'&&$common.havePower(119)" @click="handleClick(scope.row,3)"  type="text" size="mini">删除</el-button>
                <el-button v-if="$common.havePower(118)" @click="handleClick(scope.row,5)"  type="text" size="mini">上架</el-button>
              </template>
              <el-button v-if="scope.row.goodsStatus!=='未上架'&&$common.havePower(50)" @click="handleClick(scope.row,4)" type="text" size="mini">下架申请</el-button>
              <el-button v-if="(scope.row.goodsProperty==='虚拟质押'||scope.row.goodsProperty==='质押商品')&&scope.row.goodsStatus==='未上架'&&$common.havePower(120)"  @click="handleClick(scope.row,6)" type="text" size="mini">解除质押</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block">
          <el-pagination
            ...
            <div style="display: inline-block">
              <el-button @click="handleMultiple" type="text" size="mini">批量下架申请</el-button>
            </div>
          </el-pagination>
        </div>
      </div>
    ...
    <div v-else key="showDetail2">
      ...
      <div>
        <el-form :disabled="!controlType.btn" size="mini"  :inline="true" :model.sync="ruleForm" :rules="rules" ref="ruleForm" status-icon>
          ...
          <template v-if="controlType.btn==='上架'||+ruleForm.status!==0">
            <el-form-item prop="goodsGiveMoney" label="分红金额：">
              <el-input  :maxlength=13  v-model="ruleForm.goodsGiveMoney" placeholder="">
                <template slot="suffix">元</template>
              </el-input>
            </el-form-item>
            <el-form-item prop="goodsReward" label="奖励金额：">
              <el-input  :maxlength=13  v-model="ruleForm.goodsReward" placeholder="">
                <template slot="suffix">元</template>
              </el-input>
            </el-form-item>
          </template>
          ...
          <el-table v-if="goodsFormatData.length" style="margin-top: 10px"     border   max-height="400"   :data="goodsFormatData">
            <el-table-column v-for="(item,index) in goodsFormatForm" v-if="index!=='id'" :key="index" align="center" :prop="index" :label="index === '零售价'||index === '上架单价' ? index+'（元）' : index"></el-table-column>
            <el-table-column align="center" prop="handle" label="操作">
              <template slot-scope="scope">
          ...
          <admin-upload
            ...
            <div style="margin-bottom: 10px">
              ...
              <el-button @click="addGoodsParameters" size="mini" type="primary">保存</el-button>
            </div>
          ...
          <template  v-if="ruleForm.type*1===2||ruleForm.type*1===3">
            <p>七、质押截止日期</p>
  ...
  data () {
    ...
    return {
      pickerOptions1: {
        ...
        onPick (date) {
          if (date.maxDate) {
            var start = new Date(date.maxDate)
            start.setTime(start.getTime() + 3600 * 1000 * 24 * 7)
            that.ruleForm.fromEndTime = start
          }
        }
      },
      pickerOptions2: {
        disabledDate (time) {
          return time.getTime() < that.ruleForm.fromTime[1].getTime() + 3600 * 1000 * 24 * 7
        }
      },
      controlType: {
        title: '查看商品',
      ...
      rules: {
        ...
        pledgeEndTime: [
          {required: true, message: '请选择质押截止时间'}
        ]
      },
      multipleSelection: [],
      showDetail: false,
  ...
  methods: {
    ...
    getSkuKey (type, fn) {
      ...
      that.$axios({
        ...
        skuKey['库存量'] = ''
        skuKey['虚拟销量'] = ''
        if (that.controlType.btn === '上架' || (!that.$common.isNull(type) && +type !== 0)) {
          skuKey['上架单价'] = ''
        }
    ...
    pageData () {
      ...
      this.$axios({
        method: 'get',
        url: 'supply/getSubpplyDetail.action',
        params: {
          ...
          rows: that.pageSize,
          goodsProperty: that.ruleForm2.goodsNature,
          goodsStatus: that.ruleForm2.goodsStatus,
    ...
    addGoodsOk (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          console.log(that.goodsFormatData)
          if (!that.$common.skuCompare(that.goodsFormatData)) {
            that.$message.warning('商品规格不一致，请调整')
            return
          }
          if (that.controlType.btn === '上架') {
            var currentArr = that.goodsFormatData
            for (var i = 0, len = currentArr.length; i < len; i++) {
              if (that.$common.isNull(currentArr[i]['上架单价'])) {
                that.$message.warning('请添加上架单价')
                return
              }
            }
          }
          var option = {
            ...
            rebate: currentVal.goodsGiveMoney ? Math.round(currentVal.goodsGiveMoney * 100) : '',
            reword: currentVal.goodsReward ? Math.round(currentVal.goodsReward * 100) : '',
          ...
          that.goodsFormatData.forEach(function (val) {
            ...
            for (var key in val) {
              if (key === '零售价') {
                ...
                a.priceSales = Math.round(val[key] * 100)
              } else if (key === '库存量') {
    ...
    goodsFormatSet (formName, type) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          for (var key in that.goodsFormatForm) {
            if (that.goodsFormatForm[key] && key !== '零售价' && key !== '库存量' && key !== '上架单价') {
              flag = true
              break
            }
          }
    ...
    handleClick (data, type) {
      var that = this
      if (type === 3) {
        this.$confirm('是否删除' + data.name + '，删除后将不可恢复', '删除确认', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          that.$axios({
            method: 'post',
            url: 'supply/delItemOnSale.action',
            data: {
              itemIds: [data.id],
              userId: sessionStorage.getItem('userId')
            }
        ...
        this.$confirm('是否确认' + data.name + '下架申请，再次上架请联系管理人员', '下架确认', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          that.$axios({
            method: 'post',
            url: 'supply/setItemOnSale.action',
            data: {
              itemIds: data.id,
              userId: sessionStorage.getItem('userId')
            }
        ...
        this.$confirm('是否确认解除' + data.name + '的质押状态，解除后将不可恢复。', '解除质押确认', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
        ...
        var url = 'supply/searchSubpplyDetail.action'
        var dataOption = {
          goodsId: data.id
        }
        that.$axios({
