    ...
    <div v-if="!showDetail" key="showDetail1">
      ...
      <div>
        <div style="text-align: right;margin-bottom: 10px">
          <el-button v-if="$common.havePower(40)" @click="showDetail = true" type="primary" size="mini">新增商品</el-button>
        </div>
        <el-table  ref="multipleTable" @select-all="checked=!checked" @selection-change="handleSelectionChange" class="admin-table" border   max-height="400"   :data="tableData">
          ...
          <el-table-column align="center" prop="handle" label="操作">
            <template slot-scope="scope">
              <el-button v-if="$common.havePower(36)" @click="handleClick(scope.row,1)" type="text" size="mini">查看</el-button>
              <el-button v-if="scope.row.statusName==='未上架'&&$common.havePower(37)" @click="handleClick(scope.row,2)" type="text" size="mini">修改</el-button>
              <el-button v-if="scope.row.statusName!=='未上架'&&$common.havePower(39)" @click="handleClick(scope.row,3)" type="text" size="mini">下架申请</el-button>
              <el-button v-if="scope.row.statusName==='未上架'&&$common.havePower(38)" @click="handleClick(scope.row,4)" type="text" size="mini">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block">
          ...
          <el-table v-if="goodsFormatData.length" style="margin-top: 10px"     border   max-height="400"   :data="goodsFormatData">
            ...
            <el-table-column align="center" prop="handle" label="操作">
              <template slot-scope="scope">
                ...
                <span class="admin-btn-set" v-if="+ruleForm.status===2&&$common.havePower(211)"  @click="goodsFormatClick(scope,3)">修改库存</span>
              </template>
            </el-table-column>
          </el-table>
          <p>二、商品宣传信息</p>
...
export default {
  ...
  methods: {
    ...
    addAddress: function (dataName, formName) {
      ...
      console.log(this.selfAddressList)
