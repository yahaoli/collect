<template>
  <div>
    <div v-if="!showDetail" key="showDetail1">
      ...
      <div>
        <div style="text-align: right;margin-bottom: 10px">
          <el-button v-if="$common.havePower(33)" @click="showDetail = true" type="primary" size="mini">新增商品</el-button>
        </div>
        <el-table  ref="multipleTable" @select-all="checked=!checked" @selection-change="handleSelectionChange" class="admin-table" border   max-height="400"   :data="tableData">
          ...
          <el-table-column align="center" prop="handle" label="操作">
            <template slot-scope="scope">
              <el-button v-if="$common.havePower(30)" @click="handleClick(scope.row,1)" type="text" size="mini" >查看</el-button>
              <el-button v-if="scope.row.statusName==='未上架'&&$common.havePower(31)" @click="handleClick(scope.row,2)" type="text" size="mini">修改</el-button>
              <el-button v-if="scope.row.statusName!=='未上架'&&$common.havePower(34)" @click="handleClick(scope.row,3)" type="text" size="mini">下架申请</el-button>
              <el-button v-if="scope.row.statusName==='未上架'&&$common.havePower(32)" @click="handleClick(scope.row,4)" type="text" size="mini">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block">
    ...
    <div v-if="showDetail" key="showDetail12">
        ...
        <el-form :disabled="!controlType.btn" size="mini"  :inline="true" :model.sync="ruleForm" :rules="rules" ref="ruleForm" status-icon>
          ...
          <el-table v-if="goodsFormatData.length" style="margin-top: 10px"     border   max-height="400"   :data="goodsFormatData">
            ...
            <el-table-column align="center" prop="handle" label="操作">
              <template slot-scope="scope">
                ...
                <span class="admin-btn-set" v-if="+ruleForm.status===2&&$common.havePower(210)"  @click="goodsFormatClick(scope,3)">修改库存</span>
              </template>
            </el-table-column>
          </el-table>
          <p>二、商品宣传信息</p>
          ...
          <admin-upload
            ...
            <el-table v-if="goodsParametersData.length"  ref="goodsParametersTable"   border   max-height="400"   :data="goodsParametersData">
              ...
              <el-table-column align="center" prop="handle" label="操作">
                <template slot-scope="scope">
                  <el-button @click="handleClick2(scope,1)" type="text" :disabled="!controlType.btn" size="mini">修改</el-button>
                  <el-button v-if="scope.$index!==0" @click="handleClick2(scope,2)" :disabled="!controlType.btn" type="text" size="mini">上移</el-button>
                  <el-button v-if="scope.$index!==(goodsParametersData.length-1)" :disabled="!controlType.btn" @click="handleClick2(scope,3)" type="text" size="mini">下移</el-button>
                  <el-button @click="handleClick2(scope,4)" type="text" :disabled="!controlType.btn" size="mini">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
            <p>三、商品服务项</p>
          ...
          <el-form-item >
            <el-input  :maxlength=15  v-model="addressList.storageAddressPhone" placeholder="请输入联系人方式"></el-input>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  methods: {
    ...
    changeImg (url) {
      ...
      console.log(this.ruleForm[this.currentImg.key][this.currentImg.index])
    },
    handleCity (value) {
    ...
    handleClick (data, type) {
      var that = this
      if (type === 4) {
        this.$confirm('是否删除' + data.goodsName + '，删除后将不可恢复', '删除确认', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
        ...
        this.$confirm('是否确认' + data.goodsName + '下架，下架后需重新审核才可上架销售', '下架确认', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
