	...
	<section class="details" v-loading="loading">
		...
		<main>
			<div class="top-flex" flex="main:justify">
				<ul v-if="orderDetails.team_main_order">
					<li>
						<span>客源地：</span>
						<span> {{ orderDetails.team_city || "--" }}</span>
					</li>
				</ul>
				<ul>
					<li>
					...
					<li>
						<span>数量：</span>
						<span>原:{{ orderDetails.origin_num || "--" }}/验:{{ orderDetails.verified_num || "--" }}/退:{{ orderDetails.refund_num || "--" }}</span>
					</li>
					<li v-if="orderDetails.is_vipOrder">
						<span>折扣抵扣金额：</span>
						<span> {{ orderDetails.is_vipOrder.discount_price || "--" }}</span>
					</li>
					<li v-if="orderDetails.is_vipOrder">
						<span>积分抵扣金额：</span>
						<span> {{ orderDetails.is_vipOrder.integral_price || "--" }}</span>
					</li>
					<li>
						<span>订单备注：</span>
				...
				<ul>
					...
					<li v-if="orderDetails.main_assn">
						<span>总社：</span>
						<span>{{ orderDetails.main_assn || "--" }}</span>
					</li>
